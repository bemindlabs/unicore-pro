/**
 * @unicore/domains — SSL provisioning unit tests
 *
 * Tests cover:
 *  - SSL type definitions and constants
 *  - SslModule static registration
 *  - CloudflareSslClient HTTP interactions (mocked fetch)
 *  - SslService provisioning logic with mock Prisma + mock CF client
 *  - SslService checkCertificateStatus
 *  - SslService renewCertificate
 *  - SslMonitorService runCheck cycle (expiry detection)
 *  - SSL_EVENTS constants
 */
...
function makeOptions(overrides: Partial<SslModuleOptions> = {}): SslModuleOptions {
  return {
    cloudflareApiToken: 'test_cf_token',
    defaultSslMode: 'full',
...
function makeCertRecord(overrides: Partial<SslCertificate> = {}): SslCertificate {
  return {
    ...
    lastCheckedAt: new Date('2026-03-01'),
    renewalAttempts: 0,
    errorMessage: null,
    cloudflareCertPackId: 'pack_01',
...
describe('SSL_EVENTS', () => {
  it('exports PROVISIONED event name', () => {
    assert.equal(SSL_EVENTS.PROVISIONED, 'ssl.provisioned');
  });

  it('exports EXPIRY_WARNING event name', () => {
    assert.equal(SSL_EVENTS.EXPIRY_WARNING, 'ssl.expiry.warning');
  });

  it('exports EXPIRED event name', () => {
    assert.equal(SSL_EVENTS.EXPIRED, 'ssl.expired');
  });

  it('exports RENEWED event name', () => {
    assert.equal(SSL_EVENTS.RENEWED, 'ssl.renewed');
  });

  it('exports ERROR event name', () => {
    assert.equal(SSL_EVENTS.ERROR, 'ssl.error');
  });
});
...
describe('ssl.constants', () => {
  it('SSL_MODULE_OPTIONS is the expected injection token', () => {
    assert.equal(SSL_MODULE_OPTIONS, 'SSL_MODULE_OPTIONS');
  });

  it('DEFAULT_EXPIRY_WARNING_DAYS is 30', () => {
    assert.equal(DEFAULT_EXPIRY_WARNING_DAYS, 30);
  });

  it('DEFAULT_MONITOR_INTERVAL_MS is 1 hour', () => {
    assert.equal(DEFAULT_MONITOR_INTERVAL_MS, 3_600_000);
  });
...
describe('SslCertificate model shape', () => {
  it('cert record has all required fields', () => {
    ...
    assert.ok(
      ['pending', 'active', 'expiring', 'expired', 'error'].includes(cert.status),
    );
...
// ---------------------------------------------------------------------------
// SslModule (NestJS dynamic module — smoke test)
// ---------------------------------------------------------------------------

describe('SslModule', () => {
  async function tryImport() {
    try {
      const mod = await import('../ssl.module.js');
      return mod;
    } catch {
      return null;
    }
  ...
  it('exports register() and registerAsync() static methods', async () => {
    ...
    if (!mod) return; // skip if @nestjs/common not installed
  ...
  it('registerAsync() accepts useFactory', async () => {
    const mod = await tryImport();
    if (!mod) return;
    const dm = mod.SslModule.registerAsync({
...
// ---------------------------------------------------------------------------
// CloudflareSslClient — mocked fetch
// ---------------------------------------------------------------------------
...
describe('CloudflareSslClient', () => {
  async function buildClient(fetchImpl?: typeof fetch) {
    ...
    // Construct manually (bypassing NestJS DI) by calling the class constructor
    // We exploit the fact that the Inject decorator does NOT prevent direct construction.
    const client = new (CloudflareSslClient as unknown as {
      new (opts: SslModuleOptions): InstanceType<typeof CloudflareSslClient>;
    })(options);

    // Patch the private fetch calls via globalThis shim
    if (fetchImpl) {
  ...
  it('getSslMode returns the mode string from the CF response', async () => {
    const client = await buildClient(
      () => makeCfSuccess({ id: 'ssl', value: 'strict', modified_on: '2026-01-01' }),
    );
  ...
  it('setSslMode throws on Cloudflare API failure', async () => {
    ...
    await assert.rejects(
      () => client.setSslMode('zone_bad', 'full'),
      /setSslMode failed/,
    );
  });
  ...
  it('orderAdvancedCertPack returns id+status on success', async () => {
    const client = await buildClient(
      () => makeCfSuccess({ id: 'pack_new', status: 'initializing' }),
    );
    const result = await client.orderAdvancedCertPack('zone_123', 'example.com');
  ...
  it('getSslDetails maps pack to CloudflareSslDetails shape', async () => {
    ...
    assert.equal(details.primaryCertificate?.validTo, '2027-01-01T00:00:00Z');
  });
});

// ---------------------------------------------------------------------------
// SslService — mocked Prisma + CF client
// ---------------------------------------------------------------------------
...
describe('SslService', () => {
  ...
  function makeMockPrisma(overrides: {
    ...
    ssl?: SslCertRecord | null;
  } = {}) {
    const certStore: Record<string, SslCertRecord> = {};

    // Seed from explicit ssl override
    if (overrides.ssl) {
      certStore[overrides.ssl.domainId] = overrides.ssl;
    }

    // Also seed from domain.ssl when provided (so update() finds the record)
    if (overrides.domain?.ssl) {
      certStore[overrides.domain.ssl.domainId] = overrides.domain.ssl;
    }
    ...
    return {
      ...
      sslCertificate: {
        ...
        findUnique: async ({ where }: { where: { domainId: string } }) => {
          return certStore[where.domainId] ?? null;
        },
  ...
  function makeDbDomain(overrides: Partial<DomainRecord> = {}): DomainRecord {
    return {
      ...
      tenantId: 'tenant_01',
      status: 'active',
      cloudflareZoneId: 'zone_abc',
      ssl: null,
      ...overrides,
    };
  }

  // Build a mock CF client
  ...
  async function buildService(opts: {
    ...
    cfClient?: ReturnType<typeof makeMockCfClient>;
    options?: SslModuleOptions;
  } = {}) {
    const { SslService, SSL_PRISMA } = await import('../services/ssl.service.js');
    ...
    // Directly instantiate (bypassing DI) — the @Inject decorators don't affect
    // direct constructor calls; they only add metadata for the NestJS container.
    const service = new (SslService as unknown as {
      new (
        options: SslModuleOptions,
        prisma: unknown,
        cfClient: unknown,
        emitter: unknown,
      ): InstanceType<typeof SslService>;
    })(options, prisma, cfClient, mockEmitter);

    void SSL_PRISMA; // ensure export is referenced (avoids tree-shake in tests)
    return service;
  }
  ...
  it('provisionSsl: throws NotFoundException when domain not found', async () => {
    ...
    await assert.rejects(
      () => svc.provisionSsl({ domainId: 'dom_missing', hostname: 'x.com', cloudflareZoneId: 'z' }),
      /dom_missing/,
    );
  });
  ...
  it('provisionSsl: activates Cloudflare when universal SSL pack is active', async () => {
    ...
    const result = await svc.provisionSsl({
      domainId: 'dom_01',
      hostname: 'example.com',
      cloudflareZoneId: 'zone_abc',
    }) as SslProvisionResult;
  ...
  it('provisionSsl: falls back to Let\'s Encrypt when Cloudflare unavailable', async () => {
  ...
  it('checkCertificateStatus: throws NotFoundException for unknown domain', async () => {
    ...
    await assert.rejects(
      () => svc.checkCertificateStatus('dom_missing'),
      /dom_missing/,
    );
  });
  ...
  it('checkCertificateStatus: computes daysUntilExpiry correctly', async () => {
    ...
    const prisma = makeMockPrisma({
      domain: makeDbDomain({ ssl: cert, cloudflareZoneId: null }),
    });
    const svc = await buildService({ prisma });
    ...
    assert.ok(!status.isExpiringSoon); // 60 days > 30-day threshold
  });
  ...
  it('renewCertificate: succeeds for Let\'s Encrypt provider', async () => {
    const cert: SslCertRecord = {
      id: 'cert_03',
      domainId: 'dom_01',
      provider: 'letsencrypt',
  ...
  it('renewCertificate: returns Cloudflare auto-managed message for Universal SSL', async () => {
    ...
    const prisma = makeMockPrisma({
      domain: makeDbDomain({ ssl: cert }),
    });
    // CF client returns null for ACM order (plan limitation) → Universal SSL message
  ...
  // Using a simple type alias to avoid duplicate interface warnings in the test file
  type SslCertRecord = {
    id: string;
    domainId: string;
    provider: string;
...
describe('SslMonitorService', () => {
  async function buildMonitor(opts: {
    ...
    const mockSslService = {
      findAll: async () => certs,
      checkCertificateStatus: checkFn,
    };
    ...
    const monitor = new (SslMonitorService as unknown as {
      new (
        opts: SslModuleOptions,
        svc: unknown,
        emitter: unknown,
      ): InstanceType<typeof SslMonitorService>;
    })(options, mockSslService, emitter);
  ...
  it('runCheck: emits EXPIRY_WARNING for cert expiring in < 30 days', async () => {
    ...
    const result = await monitor.runCheck();
    assert.ok(result.warnings >= 1, 'should report at least 1 warning');
  ...
  it('runCheck: emits EXPIRED for a certificate that has already expired', async () => {
    const cert = makeCertRecord({
      domainId: 'dom_02',
      status: 'expired',
  ...
  it('runCheck: healthy cert with null expiresAt produces ok result', async () => {
    ...
    const { monitor, emitter } = await buildMonitor({
      certs: [cert],
      checkFn: async (id: string) => ({ hostname: `${id}.example.com`, expiresAt: null }),
    });
  ...
  it('stopMonitoring: clears the timer gracefully', async () => {
    ...
    // Start and immediately stop — no errors
...
// ---------------------------------------------------------------------------
// SslController health endpoint (pure function test)
// ---------------------------------------------------------------------------
...
describe('SslController — health response shape', () => {
  it('health response contains all required fields', () => {
    ...
    assert.ok(typeof healthResponse.hostname === 'string');
    assert.ok(typeof healthResponse.isValid === 'boolean');
    assert.ok(typeof healthResponse.status === 'string');
    assert.ok(typeof healthResponse.provider === 'string');
    assert.ok(healthResponse.expiresAt instanceof Date);
    assert.ok(typeof healthResponse.daysUntilExpiry === 'number');
    assert.ok(typeof healthResponse.isExpiringSoon === 'boolean');
    assert.ok(healthResponse.checkedAt instanceof Date);
  });
